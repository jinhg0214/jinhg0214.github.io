...
2. 해당 벽을 부쉈을 때, 그룹이 몇개 있는지 체크하는 파트 
...
- 서로 다른 구역만 더하기 위해서, 각 벽에 대해 인접한 구역 번호를 중복없이 관리
...
`map.resize(N, vector<int>(M)); group.resize(N, vector<int>(M, -1));`과 같이 2중 벡터를 이용한 맵 관리나

`auto [now_y, now_x] = qu.front()` 과 같이 구조적 바인딩(structured bindings)을 사용하는 방법도 배울 수 있었다